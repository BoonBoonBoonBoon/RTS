...
private:
	...
	static void HandleTypes(const TArray<AActor*>& UnitArray, const AActor* UnitActor);

	// Pulls the specific Unit ID Type.
	static EUnitTypes GetUnitType(const AActor* UnitActor);

	static void DefineAttributes(const AActor* UnitActor, EUnitTypes UnitTypes);
	...
	static TArray<EUnitAttributes> GetAttributesForunit(EUnitTypes UnitTypes); // chucking errors here

	/*// Finds the attributes of a Specific unit.
	static TArray<EUnitAttributes> FindUnitAttributes(EUnitTypes UnitTypes, TArray<EUnitAttributes>& UnitAttributes);
